...
9. `d.healPlace(ctx)` (Кулак Хаоса or estate, depending on `useDukeEstate`), `d.wheatFields(ctx)` (estate approach), `d.autoban(ctx)` (7.5s reload), `d.enterBattle(ctx)` ("В бой"), `d.pathPending(ctx)` ("путь лежит" → `update(6100)`), `d.work(ctx)` (работа/rudnik fort handling, refresh mode from `rudnikRefresh`).
...
- [src/game/drivers.js](server/src/game/drivers.js) — `DRIVERS` registry (`key → {file, label}`) that drives the bot's driver picker and the script loader, plus `DRIVER_EXTRA_SETTINGS` (per-driver custom localStorage settings; currently only bleyk's `expo`). Adding a driver script means adding it here; adding a driver-specific setting means an entry here **and** a toggle in `menus.js`/`bot/index.js`.
