...
5. Handle hometown detection (Центральная площадь / поднятый в / Северо-западный форпост, keyed off the alignment-derived `hometown` id): decide between going to heal (`myHP <= houseHP`, to Кулак Хаоса via `lway=4` or duke estate via `konj&lway=22`), resting in place when reserves are short, going to the bot (`myHP >= goHP`), or scheduling a refresh via `utils.update(...)`.
...
7. Target-reached block: attack if HP ok **and reserves are not short**, else teleport home.
8. Tail handlers: автобан reload, "В бой" entry, работа/rudnik (fort) interleaved handling, fallback "go home or go to target" default (also reserve-gated).

**Reserve gating (added alongside HP):** every driver reads `utils.getReserves(str)` at exactly the decision branches that matter — hometown, target-reached, the Кулак Хаоса/estate heal block, and the fallback default — and refuses to engage the bot when reserves are short. It is **not** read on the intermediate walking/work branches, so the chat fallback never fires there. Priority at each branch is **heal (critically low HP, `myHP <= houseHP`) > rest (reserves short) > fight (`myHP >= goHP`) > mild HP waits**, so deeply negative HP still routes to fast healing regardless of reserves. When short on reserves in the hometown the driver sleeps in place via `utils.update(...)` (same interval as the `устали` handler) instead of portaling to the bot; at any other location it teleports home with a foreground `location.href` and lets the hometown branch sleep. A `getReserves` result of `null` (unparseable even via chat) disables the gate — behaviour falls back to the legacy flow where the in-game `устали` fatigue message triggers a **background** teleport home, so that handler must stay as the safety net. The standard bots gate on `reserves < 0`; **lbast_paladin** gates every engage decision on `reserves < 5` (so a kill can't leave the quest uncompletable), sleeps 4–8 min (`rand * 480`) when reserves are in `[0,5)` and the full interval when negative, and folds its `Продолжить квест` click into the `стоит склеп` crypt block (gated on co-occurrence with that text) so only this quest's continue-link is followed — other quests' continue-links fall through to the default branch.
